...
if __name__ == "__main__":
    ...
    for cat_comb in itertools.combinations(numeric_feats, 2):
        i,j = cat_comb
        feat = i+"_"+j
        train_test[feat] = np.sqrt(preprocessing.minmax_scale(train_test[i] + train_test[j]))
        print feat 
    ...
    cats = [x for x in train_test.columns if 'cat' in x]
    for col in cats:
        train_test[col] = pd.factorize(train_test[col], sort=True)[0]
#        print train_test[feat].dtype
    ...
    if kfolds > 1:
        ...
        for i, (train_index, test_index) in enumerate(kf):
            ...
            gbdt = xgb.train(xgb_params, dtrain, best_nrounds, watchlist,
                             ...
                             verbose_eval=50,
                             early_stopping_rounds=25)
            train_sub[test_index] = gbdt.predict(dvalid, ntree_limit=gbdt.best_ntree_limit)
            del dtrain
            del dvalid
            gc.collect()
